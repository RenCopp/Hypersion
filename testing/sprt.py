...
DEFAULT_CUTECHESS = Path(
    os.environ.get("CUTECHESS", shutil.which("cutechess-cli") or "cutechess-cli")
)
...
DEFAULT_OPENINGS  = Path(
    os.environ.get(
        "HYPERSION_OPENINGS",
        str(HERE / "openings" / "popularpos_lichess_v3.epd"),
    )
)
...
def build_command(args, pgn_path: Path) -> list[str]:
    ...
    # `--no-nnue` forces classical eval by unloading both NNUE nets per
    # engine. Useful for testing pure-classical changes (e.g. eval_params,
    # PSQT tables) where the NNUE forward pass would mask the signal.
    if getattr(args, "no_nnue", False):
        each_opts += ["option.EvalFile=<empty>", "option.EvalFileSmall=<empty>"]
    ...
    new_threads = args.new_threads or args.threads
    old_threads = args.old_threads or args.threads
    cmd = [str(args.cutechess)]
    cmd += [
        "-engine", "name=NEW", f"cmd={args.new}", "option.OwnBook=false",
        f"option.Threads={new_threads}",
        "-engine", "name=OLD", f"cmd={args.old}", "option.OwnBook=false",
        f"option.Threads={old_threads}",
        "-each", *each_opts,
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--new-threads", type=int, default=None,
                   help="candidate thread count (overrides --threads)")
    p.add_argument("--old-threads", type=int, default=None,
                   help="baseline thread count (overrides --threads)")
    ...
    p.add_argument("--no-nnue", action="store_true",
                   help="Pass EvalFile=<empty> + EvalFileSmall=<empty> to both "
                        "engines so NNUE is unloaded -- pure classical eval test.")
