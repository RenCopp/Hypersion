@echo off
REM Mirror gauntlet v2: Hypersion_v1 vs same opponents at same harsher handicaps.
...
"%CC%" ^
  ...
  -pgnout "%~dp0gauntlet2_v1_vs_sf2400.pgn"

echo === V1 vs Obsidian 16 (TC handicap %TC_HEAVY%) ===
"%CC%" ^
  -engine name=Hyp_V1 cmd="%HYPER%" tc=%TC_HYPER% ^
  ...
  -openings file="%OPENINGS%" format=epd order=random plies=8 ^
  -repeat -recover -concurrency %CONC% -games %GAMES% -ratinginterval 5 ^
  -pgnout "%~dp0gauntlet2_v1_vs_obsidian.pgn"

echo === V1 vs Alexandria 9 (TC handicap %TC_HEAVY%) ===
"%CC%" ^
  -engine name=Hyp_V1 cmd="%HYPER%" tc=%TC_HYPER% ^
  ...
  -openings file="%OPENINGS%" format=epd order=random plies=8 ^
  -repeat -recover -concurrency %CONC% -games %GAMES% -ratinginterval 5 ^
  -pgnout "%~dp0gauntlet2_v1_vs_alexandria.pgn"

echo === V1 vs RubiChess (LimitNps=10000) ===
"%CC%" ^
  -engine name=Hyp_V1 cmd="%HYPER%" tc=%TC_HYPER% ^
  ...
  -openings file="%OPENINGS%" format=epd order=random plies=8 ^
  -repeat -recover -concurrency %CONC% -games %GAMES% -ratinginterval 5 ^
  -pgnout "%~dp0gauntlet2_v1_vs_rubichess.pgn"

echo === V1 gauntlet v2 complete ===
endlocal
